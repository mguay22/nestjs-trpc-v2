...
<Callout>
You must call the `AppRouterHost.appRouter` getter after the application has been initialized (after the `onModuleInit` hook has been triggered by either the `app.listen()` or `app.init()` method).
</Callout>
...
Here is a short example of how to integrate [trpc-panel](https://github.com/iway1/trpc-panel), while this is a specific integration, the techniques used here can be applied to any of the trpc-specific libraries you wish to include.

If you still run into issues, please open a [new issue](https://github.com/mguay22/nestjs-trpc-v2/issues/new) or contact us via Discord.
...
```typescript title="trpc-panel.controller.ts"
...
<Callout>
  As you can see in the example above, we are using `onModuleInit` lifecycle
  method to make sure the appRouter is initialized and available.
</Callout>
  </Step>
